...
void callback(char* topico, byte* payload, unsigned int length) {
  ...
  if (disp == '1' && sens == '2' && state == '1') {
    ...
    digitalWrite(D6, LOW);  
    snprintf (msg, MSG_BUFFER_SIZE, "{\"device\":\"1\",\"sensor\":\"%.c\",\"value\":%.c,\"user\":%.c ,\"action\":\"2\"}",(sens),(state),(user));
    Serial.print(msg); 
...
//  Serial.println(t);
    snprintf (msg, MSG_BUFFER_SIZE, "{\"device\":\"1\",\"sensor\":\"%.i\",\"valor\":%.2f,\"action\":\"1\"}",sensor_id, t);
